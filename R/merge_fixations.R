...
#'
#' @param df A data frame containing fixation information with columns 'eye', 't_start', t_end', 'duration', 'x', 'y', and 'pupil'.
#' @param max_duration The maximum duration in milliseconds for a fixation to be considered for merging (default is 200 ms).
#' @param max_distance The maximum distance in pixels between fixations for them to be considered for merging (default is 50 pixels).
#' @param rate The sampling rate of the eye tracker in Hz (default is 1000 Hz).
#' @param add_eyelink_correction A logical indicating whether to add a correction for the
#'
...
merge_fixations <-
  function(df,
    ...
    # add columns for previous and next fixation
